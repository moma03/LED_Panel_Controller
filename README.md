...
1. Clone with submodules (or run `git submodule update --init --recursive` if already cloned):
   ...
   under `third_party/rpi-rgb-led-matrix`, which the `idle`/`shutdown`
...
`system.idle` and `system.shutdown` in `config.yaml` point at
[`programs/idle.py`](programs/idle.py) and [`programs/shutdown.py`](programs/shutdown.py):
...
Starting a program while one is already running doesn't play any in-between
animation — `TransitionManager.start()` just stops whatever's in the foreground
(idle animation or the active program) and launches the new one directly.
...
`system.shutdown`, or any program under `programs:` — and the
